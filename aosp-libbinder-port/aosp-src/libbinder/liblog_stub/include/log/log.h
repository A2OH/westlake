...
// IF_ALOG passes `priority` to another macro that token-pastes it; that
// triggers pre-expansion in glibc/bionic where LOG_INFO etc. may be
// defined to integers by <syslog.h>.  To avoid this, evaluate the priority
// token-paste once here and use the resolved enum value in both the
// runtime check and __android_log_print arg, bypassing IF_ALOG's own paste.
#define ALOG(priority, tag, fmt, ...)                                        \
    do {                                                                     \
        if (false)[[/*VERY*/ unlikely]] { /* ignore unused __VA_ARGS__ */    \
            std::fprintf(stderr, fmt __VA_OPT__(, ) __VA_ARGS__);            \
        }                                                                    \
        if (__android_log_stub_is_loggable(ANDROID_##priority) &&            \
