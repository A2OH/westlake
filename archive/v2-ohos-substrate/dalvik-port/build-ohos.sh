...
check_toolchain() {
    if [ ! -x "$OHOS_LLVM/bin/clang++" ]; then
        ...
        echo "Expected: OpenHarmony prebuilts at $HOME/openharmony/"
        exit 1
    fi
...
build_sysroot() {
    ...
    OH_KERNEL_HEADERS="$HOME/openharmony/kernel/linux/patches/linux-5.10/prebuilts/usr/include"
    ...
    KERNEL="$HOME/openharmony/kernel/linux/linux-5.10"
    if [ -d "$KERNEL" ]; then
...
build_libffi() {
    ...
    LIBFFI_SRC="$HOME/openharmony/third_party/libffi"
...
build_zlib() {
    ...
    ZLIB_SRC="$HOME/openharmony/third_party/zlib"
    if [ ! -d "$ZLIB_SRC" ]; then
