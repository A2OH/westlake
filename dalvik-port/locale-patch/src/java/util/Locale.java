...
public final class Locale implements Cloneable, Serializable {
    ...
    // ------------------------------------------------------------------------
    // CR-Y+1 (2026-05-15): BCP-47 language-tag support back-ported from Java 7
    // (libcore API-21). The dalvik-kitkat libcore omits these methods; AndroidX
    // libraries (notably emoji2.attachBaseContext, AppCompat resource configs)
    // call them directly and crash with NoSuchMethodError on this BCP. These
    // are pure additive AOSP-default-equivalent implementations — they touch
    // only the public surface and the existing private fields, and have no
    // dependency on libcore.icu beyond what the existing class already uses.
    //
    // The grammar we accept is the subtag-prefix subset of RFC 5646:
    //     tag        := language [ "-" script ] [ "-" region ] [ "-" variant... ]
    //     language   := 2*3ALPHA  (e.g. "en", "fil")
    //     script     := 4ALPHA    (e.g. "Hant")  -- swallowed (no script field
    //                                              in this Locale; preserved
    //                                              via variant if no variant
    //                                              already present)
    //     region     := 2ALPHA / 3DIGIT
    //     variant    := 5*8alnum / DIGIT 3alnum
    //
    // Extensions (-u-, -x-, -t-) and grandfathered tags fall through to
    // IllegalArgumentException — emoji2 catches this and continues with the
    // default locale, which is the AOSP-default behavior on parse failure for
    // any caller that doesn't pass strict tags.
    //
    // No per-app branches. Pure RFC 5646 prefix parser.
    // ------------------------------------------------------------------------

    /**
     * Returns a well-formed IETF BCP 47 language tag representing this locale.
     * AOSP-default equivalent for the subset of {@link Locale} state that
     * dalvik-kitkat supports (language, region/country, variant).
     *
     * @since 1.7 (API 21 on Android; back-ported here for CR-Y+1)
     */
    public String toLanguageTag() {
        // Empty Locale -> "und" per RFC 5646 / AOSP.
        if (languageCode.isEmpty() && countryCode.isEmpty() && variantCode.isEmpty()) {
            return "und";
        }
        StringBuilder sb = new StringBuilder();
        // Language subtag: AOSP rewrites "iw"->"he", "in"->"id", "ji"->"yi"
        // when emitting BCP-47 tags. Our internal storage rewrites in the
        // OTHER direction (Locale ctor maps he->iw etc.), so undo it here.
        ...
        if (!variantCode.isEmpty()) {
            // BCP-47 variants must be 5-8 alphanumeric chars, or a digit
            // followed by 3 alphanumerics. If our variant is shorter (common
            // for ad-hoc Locales like "POSIX") we still emit "-x-<variant>"
            // private-use form so the tag round-trips through forLanguageTag.
            if (isWellFormedVariant(variantCode)) {
                sb.append('-').append(variantCode);
            } else {
                sb.append("-x-").append(scrubPrivateUse(variantCode));
            }
        }
        return sb.toString();
    }

    private static boolean isWellFormedVariant(String s) {
        int len = s.length();
        if (len < 4 || len > 8) {
            return false;
        }
        // 5-8 alphanumerics, OR digit + 3 alphanumerics.
        if (len >= 5) {
            for (int i = 0; i < len; i++) {
                if (!isAlnum(s.charAt(i))) return false;
            }
            return true;
        }
        // len == 4: must be digit + 3 alphanumerics.
        char c0 = s.charAt(0);
    ...
    private static String scrubPrivateUse(String s) {
        StringBuilder out = new StringBuilder(s.length());
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            out.append(isAlnum(c) ? c : '-');
        }
        return out.toString();
    }
    ...
    /**
     * Parses a well-formed BCP 47 language tag into a {@link Locale}.
     *
     * <p>Subtags are parsed in order: primary language, optional script (4 alpha,
     * SWALLOWED — dalvik-kitkat {@code Locale} has no script field), optional
     * region (2 alpha / 3 digit), optional variants (concatenated with '_').
     *
     * <p>Extensions (-u-..., -x-..., -t-...) are consumed but discarded except
     * for "-x-" private-use which, when no other variant has been supplied,
     * becomes the {@code variant} (so round-trip through {@link #toLanguageTag()}
     * is at least lossy-stable for the cases AOSP cares about).
     *
     * <p>The empty / null tag maps to {@link #ROOT} per AOSP convention.
     *
     * @since 1.7 (API 21 on Android; back-ported here for CR-Y+1)
     */
    public static Locale forLanguageTag(String tag) {
        ...
        // "und" is the BCP-47 "undetermined language" tag, used by AOSP as
        // the round-trip target for Locale.ROOT.
        if (tag.equals("und")) {
            return ROOT;
        }
        // Split on '-'. AOSP also accepts '_' here per the back-compat
        // contract (matches Java SE 7 Locale.forLanguageTag behavior on
        // malformed input).
        ...
        // Parse: language [ script ] [ region ] [ variant... ] [ extensions... ]
        int idx = 0;
        String language = parts[idx++];
        // Validate primary language: 2-3 ALPHA (4-8 ALPHA reserved/unused).
        // If the primary subtag isn't a clean language, fall back to ROOT —
        // matches AOSP behavior on malformed tags.
        int langLen = language.length();
        if (langLen < 2 || langLen > 8 || !isAllAlpha(language)) {
            return ROOT;
        }
        // RFC 5646: "und" as primary -> Locale.ROOT (already handled above for
        // the whole-string case; if it's just the language subtag with other
        // parts, treat as unknown but still attempt to parse the rest).
        String script = "";
        if (idx < parts.length && parts[idx].length() == 4 && isAllAlpha(parts[idx])) {
            script = parts[idx++];
        }
        String region = "";
        if (idx < parts.length) {
            String maybeRegion = parts[idx];
            if ((maybeRegion.length() == 2 && isAllAlpha(maybeRegion))
                    || (maybeRegion.length() == 3 && isAllDigit(maybeRegion))) {
                region = maybeRegion;
                idx++;
            }
        }
        // Collect variants. RFC 5646: each variant is 5-8 alnum, or digit +
        // 3 alnum. We tolerate looser matches (anything that's not a 1-char
        // extension singleton).
        StringBuilder variantBuf = new StringBuilder();
        while (idx < parts.length) {
            String p = parts[idx];
            if (p.length() == 1) {
                // Extension or private-use singleton. Capture private-use
                // tail into variant; skip 'u'/'t' extensions entirely.
                if (p.equals("x") || p.equals("X")) {
                    idx++;
                    StringBuilder privTail = new StringBuilder();
                    while (idx < parts.length) {
                        if (privTail.length() > 0) privTail.append('-');
                        privTail.append(parts[idx++]);
                    }
                    if (variantBuf.length() == 0) {
                        variantBuf.append(privTail);
                    }
                    break;
                }
                // 'u' / 't' / other extension: swallow tail until next
                // singleton or end.
                idx++;
            ...
            // Otherwise treat as a variant subtag.
            if (variantBuf.length() > 0) variantBuf.append('_');
            variantBuf.append(p);
            idx++;
        }
        String variant = variantBuf.toString();

        // Script is discarded (no field in this Locale model). If you need to
        // preserve it, drop it into the variant when no other variant
        // exists. We do NOT do that here to avoid surprising AOSP callers
        // that round-trip Locale("en","US") <-> "en-US".

        // ROOT shortcut: AOSP returns Locale.ROOT for the canonical empty
        // form. Construct via the public ctor so all the kitkat-specific
        // normalisation (he->iw, etc.) runs.
        try {
    ...
    private static String[] splitTag(String tag) {
        // Hand-rolled split on '-' or '_' (also tolerates trailing
        // separators). Avoids regex / String.split overhead and pulls a
        // null-free path through.
        int len = tag.length();
        int parts = 1;
        for (int i = 0; i < len; i++) {
            char c = tag.charAt(i);
        ...
        int start = 0;
        for (int i = 0; i <= len; i++) {
            if (i == len || tag.charAt(i) == '-' || tag.charAt(i) == '_') {
                out[idx++] = tag.substring(start, i);
                start = i + 1;
            }
        }
        // Drop empty trailing/leading parts in-place.
        int real = 0;
        ...
        if (real == out.length) return out;
        String[] trimmed = new String[real];
        System.arraycopy(out, 0, trimmed, 0, real);
        return trimmed;
    }
}
