...
2. If not, the minimal build (`qemu-arm64-linux-min`) may not include graphics. Need to either:
   - Add `graphic_2d` subsystem to the product config at `$HOME/openharmony/vendor/ohemu/qemu_arm64_linux_min/config.json`
...
### OHOS ARM64 QEMU
- Kernel: Linux 5.10 ARM64 at `$HOME/openharmony-arm64/images/Image`
- System: `/tmp/ohos-arm64-images/system.img` (OHOS minimal)
- Userdata with ART: `/tmp/ohos-arm64-images/userdata-art.img`
- QEMU: `$HOME/openharmony/tools/qemu-extracted/usr/bin/qemu-system-aarch64`

### Boot command
```bash
QEMU_DIR=$HOME/openharmony/tools/qemu-extracted/usr
...
```bash
OHOS_CXX=$HOME/openharmony/prebuilts/clang/ohos/linux-x86_64/llvm/bin/clang++
OHOS_SYSROOT=$HOME/android-to-openharmony-migration/dalvik-port/ohos-sysroot
$OHOS_CXX --target=aarch64-linux-ohos --sysroot=$OHOS_SYSROOT \
  -shared -fPIC -O2 -I$COMPAT \
  -I$HOME/aosp-android-11/libnativehelper/include_jni \
  -o liboh_bridge.so arkui_bridge.cpp
```

### Rebuild OHOS with graphics (if needed)
Edit `$HOME/openharmony/vendor/ohemu/qemu_arm64_linux_min/config.json`, add:
```json
{ "subsystem": "graphic", "components": [
    { "component": "graphic_2d", "features": [] }
]}
```
Then: `cd $HOME/openharmony && ./build.sh --product-name qemu-arm64-linux-min --ccache --no-prebuilt-sdk -j8`
