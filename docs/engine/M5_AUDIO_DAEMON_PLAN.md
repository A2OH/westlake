...
Key API surface (from `$HOME/openharmony/interface/sdk_c/multimedia/audio_framework/audio_renderer/native_audiorenderer.h`):
...
6. `$HOME/aosp-android-11/frameworks/av/media/libaudioclient/include/media/IAudioFlinger.h` — interface header (Android 11; verify against framework.jar)
7. `$HOME/aosp-android-11/frameworks/av/media/libaudioclient/IAudioFlinger.cpp` — Bp/Bn skeleton to derive from
8. `$HOME/aosp-android-11/frameworks/av/media/audioserver/main_audioserver.cpp` — main() shape (strip the fork/media.log/HIDL)
9. `art-latest/stubs/audiosystem_jni_stub.cc` — M5-PRE work this depends on
10. `$HOME/openharmony/interface/sdk_c/multimedia/audio_framework/audio_renderer/native_audiorenderer.h` — Phase 2 target API (for M11 forward-planning only)
