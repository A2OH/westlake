...
2. **Verify HBC artifact tree contains `bcp/boot.art`** — `ls -la $HOME/android-to-openharmony-migration/westlake-deploy-ohos/v3-hbc/bcp/boot.art`. If missing, the artifact pull (W1) is incomplete. If present, fix `stage_push` to actually push it (currently REQUIRED_MANIFEST has it but the chroot doesn't show it).
