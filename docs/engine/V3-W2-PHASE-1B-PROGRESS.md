...
```bash
CC=$HOME/openharmony/prebuilts/clang/ohos/linux-x86_64/llvm/bin/armv7-unknown-linux-ohos-clang
SYSROOT=$HOME/openharmony/out/sdk/sdk-native/os-irrelevant/sysroot
"$CC" --sysroot="$SYSROOT" \
  -o /tmp/v3-phase1b/test_tlv_client \
  $HOME/android-to-openharmony-migration/westlake-deploy-ohos/v3-hbc/adapter-src/framework/appspawn-x/test/test_tlv_client.c
