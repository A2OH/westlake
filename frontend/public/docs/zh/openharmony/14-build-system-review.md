...
**审查的关键文件：**
- `$HOME/openharmony/build/build_scripts/build.sh`
- `$HOME/openharmony/build/build_scripts/build.py`
- `$HOME/openharmony/build/config/BUILDCONFIG.gn`
- `$HOME/openharmony/build/ohos.gni`
- `$HOME/openharmony/build/ohos_var.gni`
- `$HOME/openharmony/build/hb/main.py`
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 15-16 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/config/BUILDCONFIG.gn`，第 52、56、128、677 行
...
产品定义遵循分层模型：
- **基础配置**：`$HOME/openharmony/productdefine/common/base/`（轻量、小型、标准系统）
- **继承配置**：`$HOME/openharmony/productdefine/common/inherit/`（rich、tablet、2in1、headless 等）
- **产品配置**：`$HOME/openharmony/productdefine/common/products/`（system_arm64_default、ohos-sdk 等）
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/productdefine/common/inherit/rich.json`
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/productdefine/common/inherit/rich.json`，第 1033 行
...
- 定义在 `$HOME/openharmony/build/toolchain/ohos/BUILD.gn` 和 `ohos_toolchain.gni` 中
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 95 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/toolchain/ohos/ohos_toolchain.gni`，第 94-104 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/config/BUILDCONFIG.gn`，第 930-934 行
...
组件通过 `bundle.json` 文件定义，使用 `$HOME/openharmony/build/templates/cxx/cxx.gni` 中的模板：
- `ohos_executable` - 可执行文件
- `ohos_shared_library` - 共享库
- `ohos_static_library` - 静态库
- `ohos_source_set` - 源集

子系统注册表位于 `$HOME/openharmony/build/subsystem_config.json`，将子系统名称映射到源码路径。
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/config/sanitizers/sanitizers.gni`，第 351-405 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/config/security/security_config.gni`，第 52-78 行
...
- **严重程度：高**
- **位置**：`$HOME/openharmony/build/config/compiler/BUILD.gn`，第 1006-1016 行；`$HOME/openharmony/build/lite/config/BUILD.gn`，第 82 行
...
声明的原因是 musl libc 缺乏支持。然而，musl 具有部分 FORTIFY_SOURCE 支持，而且轻量系统配置中 `$HOME/openharmony/build/config/compiler/lite/clang/BUILD.gn:49` 和 `$HOME/openharmony/build/config/compiler/lite/gcc/BUILD.gn:49` **确实**定义了 `_FORTIFY_SOURCE=2`，造成了不一致。

#### SEC-02：x86 OHOS 的栈保护被禁用

- **严重程度：中**
- **位置**：`$HOME/openharmony/build/config/security/BUILD.gn`，第 66-67 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/config/security/security_config.gni`，第 17 行
...
- **严重程度：高**
- **位置**：`$HOME/openharmony/build/ohos_var.gni`，第 303-308 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/hb/util/post_gn/check_list.json`，第 36-47 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/config/compiler/BUILD.gn`，第 257-275 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/config/compiler/BUILD.gn`，第 298 行
...
- **严重程度：信息性**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 42、220 行
...
- **严重程度：关键**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 108、125 行
...
- **严重程度：高**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 109-110 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 117 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 106-107 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`
...
- **严重程度：信息性**
- **位置**：`$HOME/openharmony/build/build_scripts/build.py`，第 82 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/config/BUILDCONFIG.gn`，第 531-615 行
...
- **严重程度：信息性**
- **位置**：`$HOME/openharmony/build/config/compiler/BUILD.gn`，第 301 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/config/BUILDCONFIG.gn`，第 644 行
...
- **严重程度：低**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 187-194 行
...
- **严重程度：中**
- **位置**：`$HOME/openharmony/build/build_scripts/build.sh`，第 95-104 行
