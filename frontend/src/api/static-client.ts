...
export async function getSubsystem(name: string): Promise<SubsystemDetail> {
  const db = await getDb();
  const cached = queryOne(db, 'SELECT data FROM subsystem_cache WHERE name = ?', [name]);
  if (cached) return JSON.parse(cached.data);

  // Fallback for uncached subsystems
  const apiCount = queryOne(db, 'SELECT COUNT(*) as c FROM android_apis WHERE subsystem = ?', [name])!.c;
  const avgRow = queryOne(db, 'SELECT AVG(m.score) as avg_score FROM api_mappings m JOIN android_apis a ON m.android_api_id = a.id WHERE a.subsystem = ?', [name]);
  const overall = avgRow?.avg_score || 0;
  const coverage = queryOne(db, 'SELECT ROUND(100.0 * SUM(CASE WHEN m.score >= 5 THEN 1 ELSE 0 END) / COUNT(*), 1) as pct FROM android_apis a LEFT JOIN api_mappings m ON m.android_api_id = a.id WHERE a.subsystem = ?', [name])?.pct || 0;
  const scoreDist = queryAll(db, "SELECT CASE WHEN m.score >= 9 THEN 'Direct (9-10)' WHEN m.score >= 7 THEN 'Good (7-8)' WHEN m.score >= 5 THEN 'Partial (5-6)' WHEN m.score >= 3 THEN 'Hard (3-4)' ELSE 'Gap (1-2)' END as bucket, COUNT(*) as count FROM api_mappings m JOIN android_apis a ON m.android_api_id = a.id WHERE a.subsystem = ? GROUP BY bucket", [name]);
  const effortDist = queryAll(db, 'SELECT m.effort_level, COUNT(*) as count FROM api_mappings m JOIN android_apis a ON m.android_api_id = a.id WHERE a.subsystem = ? GROUP BY m.effort_level ORDER BY count DESC', [name]);
  const types = queryAll(db, 'SELECT t.id, t.name, t.kind, p.name as package_name, COUNT(a.id) as api_count, ROUND(AVG(m.score), 1) as avg_score FROM android_apis a JOIN android_types t ON a.type_id = t.id JOIN android_packages p ON t.package_id = p.id LEFT JOIN api_mappings m ON m.android_api_id = a.id WHERE a.subsystem = ? GROUP BY t.id ORDER BY api_count DESC LIMIT 50', [name]);
  const topGaps = queryAll(db, 'SELECT a.id, a.name, a.signature, m.score as compat_score, t.name as type_name, p.name as package_name FROM android_apis a JOIN android_types t ON a.type_id = t.id JOIN android_packages p ON t.package_id = p.id LEFT JOIN api_mappings m ON m.android_api_id = a.id WHERE a.subsystem = ? ORDER BY COALESCE(m.score, 0) ASC LIMIT 20', [name]);
  return { id: 0, name, api_count_android: apiCount, overall_score: overall, coverage_pct: coverage, score_distribution: scoreDist, effort_distribution: effortDist, types, top_gaps: topGaps };
}
