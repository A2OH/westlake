...
#
# Usage:
#   ./run-local-tests.sh              # run all tests (headless + UI + MockDonalds + RealAPK)
...
#   ./run-local-tests.sh realapk      # Real APK pipeline test only
...
run_realapk() {
    echo "═══ Running: Real APK Pipeline Test ═══"
    echo ""
    java -cp "$BUILD_DIR" RealApkRunner 2>&1 || true
    echo ""
}
...
case "${1:-all}" in
    ...
    realapk)      run_realapk ;;
    all)          run_headless; run_ui; run_mockdonalds; run_realapk ;;
    *)            echo "Usage: $0 {headless|ui|mockdonalds|realapk|all}"; exit 1 ;;
esac
