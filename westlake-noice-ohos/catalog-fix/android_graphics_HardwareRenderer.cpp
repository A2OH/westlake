...
static jobject android_view_ThreadedRenderer_createHardwareBitmapFromRenderNode(JNIEnv* env,
    ...
    // [OH AImageReader shim 2026-06-23] Use the REAL AImageReader NDK ABI, now
    // implemented over OHOS IConsumerSurface in liboh_adapter_bridge.so (see
    // surface_oh_helper.cpp). OHOS has no libmediandk AImageReader; the stock NDK
    // header was a compile-time no-op stub that left `window` uninitialized ->
    // RenderProxy::setSurface(garbage) -> SIGBUS. We now go through the real shim:
    // AImageReader_new -> getWindow -> render -> acquireNextImage -> AImage_getPlaneData
    // (CPU readback; GPU AHardwareBuffer import is stubbed on OHOS) -> copy into a heap
    // Bitmap. Resolved by dlsym (OH linker-namespace boundary — the adapter's sanctioned
    // cross-.so pattern). Null fallback if the shim is unavailable (software bitmap).
    typedef int oh_ms_t;
    typedef oh_ms_t (*air_new_t)(int32_t, int32_t, int32_t, int32_t, void**);
    typedef oh_ms_t (*air_getwin_t)(void*, void**);
    typedef oh_ms_t (*air_acq_t)(void*, void**);
    typedef void    (*air_rdel_t)(void*);
    typedef oh_ms_t (*aimg_plane_t)(const void*, int, uint8_t**, int*);
    typedef oh_ms_t (*aimg_stride_t)(const void*, int, int32_t*);
    typedef oh_ms_t (*aimg_dim_t)(const void*, int32_t*);
    typedef void    (*aimg_del_t)(void*);
    static air_new_t     air_new     = nullptr;
    static air_getwin_t  air_getwin  = nullptr;
    static air_acq_t     air_acq     = nullptr;
    static air_rdel_t    air_rdel    = nullptr;
    static aimg_plane_t  aimg_plane  = nullptr;
    static aimg_stride_t aimg_stride = nullptr;
    static aimg_dim_t    aimg_w      = nullptr;
    static aimg_dim_t    aimg_h      = nullptr;
    static aimg_del_t    aimg_del    = nullptr;
    if (!air_new) {
        ...
        if (h) {
            air_new     = reinterpret_cast<air_new_t>(dlsym(h, "AImageReader_new"));
            air_getwin  = reinterpret_cast<air_getwin_t>(dlsym(h, "AImageReader_getWindow"));
            air_acq     = reinterpret_cast<air_acq_t>(dlsym(h, "AImageReader_acquireNextImage"));
            air_rdel    = reinterpret_cast<air_rdel_t>(dlsym(h, "AImageReader_delete"));
            aimg_plane  = reinterpret_cast<aimg_plane_t>(dlsym(h, "AImage_getPlaneData"));
            aimg_stride = reinterpret_cast<aimg_stride_t>(dlsym(h, "AImage_getPlaneRowStride"));
            aimg_w      = reinterpret_cast<aimg_dim_t>(dlsym(h, "AImage_getWidth"));
            aimg_h      = reinterpret_cast<aimg_dim_t>(dlsym(h, "AImage_getHeight"));
            aimg_del    = reinterpret_cast<aimg_del_t>(dlsym(h, "AImage_delete"));
        }
    }
    if (!air_new || !air_getwin || !air_acq || !air_rdel || !aimg_plane || !aimg_stride
            || !aimg_w || !aimg_h || !aimg_del) {
        fprintf(stderr, "[OH-HWBMP] AImageReader shim unavailable -> null\n");
        return nullptr;
    }

    void* reader = nullptr;
    if (air_new(static_cast<int32_t>(width), static_cast<int32_t>(height),
                1 /*AIMAGE_FORMAT_RGBA_8888*/, 1, &reader) != 0 || !reader) {
        fprintf(stderr, "[OH-HWBMP] AImageReader_new failed\n");
        return nullptr;
    }
    void* winRaw = nullptr;
    if (air_getwin(reader, &winRaw) != 0 || !winRaw) {
        fprintf(stderr, "[OH-HWBMP] AImageReader_getWindow failed\n");
        air_rdel(reader);
        return nullptr;
    }
    ANativeWindow* window = reinterpret_cast<ANativeWindow*>(winRaw);

    // Render the RenderNode into the off-screen producer surface.
    {
    ...
    // Acquire the rendered image via the AImageReader shim and CPU-read its plane.
    void* image = nullptr;
    if (air_acq(reader, &image) != 0 || !image) {
        fprintf(stderr, "[OH-HWBMP] AImageReader_acquireNextImage failed\n");
        air_rdel(reader);
        return nullptr;
    }
    uint8_t* src = nullptr;
    int srcLen = 0;
    int32_t stride = 0, bw = 0, bh = 0;
    aimg_plane(image, 0, &src, &srcLen);
    aimg_stride(image, 0, &stride);
    aimg_w(image, &bw);
    aimg_h(image, &bh);
    if (!src || stride <= 0 || bw <= 0 || bh <= 0) {
        fprintf(stderr, "[OH-HWBMP] AImage plane invalid bw=%d bh=%d stride=%d\n", bw, bh, stride);
        aimg_del(image);
        air_rdel(reader);
        return nullptr;
    }
    ...
    if (!bitmap) {
        fprintf(stderr, "[OH-HWBMP] allocateHeapBitmap failed\n");
        aimg_del(image);
        air_rdel(reader);
        return nullptr;
    }

    uint8_t* dst = reinterpret_cast<uint8_t*>(bitmap->pixels());
    const size_t dstRow = bitmap->rowBytes();
    size_t copyBytes = static_cast<size_t>(width) * 4;
    copyBytes = std::min(copyBytes, static_cast<size_t>(stride));
    copyBytes = std::min(copyBytes, dstRow);
    const uint32_t rows = std::min(height, static_cast<uint32_t>(bh));
    for (uint32_t y = 0; y < rows; y++) {
        memcpy(dst + static_cast<size_t>(y) * dstRow,
               src + static_cast<size_t>(y) * stride, copyBytes);
    }
    bitmap->notifyPixelsChanged();

    // Lightweight content sanity log (sampled non-zero RGB; no file dump, no tint).
    {
        size_t nonZero = 0, sampled = 0;
        for (uint32_t y = 0; y < height; y += 2) {
            const uint32_t* row = reinterpret_cast<const uint32_t*>(dst + static_cast<size_t>(y) * dstRow);
            for (uint32_t x = 0; x < width; x += 2) { sampled++; if (row[x] & 0x00FFFFFFu) nonZero++; }
        }
        fprintf(stderr, "[OH-HWBMP] OK(AImageReader) req=%ux%u img=%dx%d stride=%d nonZeroRGB=%zu/%zu\n",
                width, height, bw, bh, stride, nonZero, sampled);
    }
...
static void android_view_ThreadedRenderer_setupShadersDiskCache(JNIEnv* env, jobject clazz,
    ...
    /* M133: egl_set_cache_filename removed */
