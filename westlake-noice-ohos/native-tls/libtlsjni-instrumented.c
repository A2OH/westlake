...
static void lgi(const char* t, int n){ char b[40]; int i=0; while(t[i]&&i<16){b[i]=t[i];i++;} b[i++]='='; int v=n,neg=0; if(v<0){neg=1;v=-v;} char d[12]; int j=0; if(v==0)d[j++]='0'; while(v){d[j++]='0'+(v%10);v/=10;} if(neg)b[i++]='-'; while(j)b[i++]=d[--j]; b[i++]='\n'; int fd=open("/data/local/tmp/tls.log",1089,438); if(fd>=0){write(fd,b,i);close(fd);} }
...
Java_com_android_internal_os_TlsJni_sslWrite(JNIEnv* env, jclass c, jlong sslp, jbyteArray buf, jint off, jint len) {
    ...
    (*env)->ReleaseByteArrayElements(env, buf, b, 2);
    lgi("nat_SSL_write", n);
    return n;
}
